<template>
      ...
      >
        ...
        <v-col>
          ...
          <v-card-text class="d-flex align-center">
            ...
            <div class="mx-4 d-flex align-center">
              <v-icon class="mr-1" small>{{ 'mdi-thumb-up' }}</v-icon>
              <span>{{ item.like_count }}</span>
            </div>
            <div class="mx-4 d-flex align-center">
              <v-icon class="mr-1" small>{{ 'mdi-star-circle' }}</v-icon>
  ...
  export default {
    ...
    methods: {
      // 获取文章列表
      async getStarArtList() {
        this.userid = window.sessionStorage.getItem('user_id')
        const { data: res } = await this.$http.get(`getstarlist/${this.userid}`, {
          params: {
